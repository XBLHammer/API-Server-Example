...
start:
	...
	if (!eNet) 
	{
		xbox::utilities::DbgOut("[Client] Xbox ethernet unplugged.");
		Sleep(1000);
		goto start;
	}
	...
	xbox::utilities::DbgOut("[Client] Ip Addr: %08X \r\n", titleAddr.ina.S_un.S_addr);
	timeoutCtr = 0;
	...
	{
		xbox::utilities::DbgOut("[Client] Xbox has no ip address");
		Sleep(1000);
		goto start;
	}
...
BOOL Connect() 
{
	...
	//{
	//	DbgPrint("[Client] #9989 XNetStartup error %d", lastSocketError);
	//	return E_FAIL;
	//}
...
start:
	...
	{
		xbox::utilities::DbgOut("[Client] #9989 unable to create socket\n");
		goto start;
	}
	...
	if (NetDll_setsockopt(XNCALLER_SYSAPP, Socket, SOL_SOCKET, SO_MARKINSECURE, (CONST PCHAR) & SockOpt, sizeof(BOOL)) != 0) 
	{
		...
		xbox::utilities::DbgOut("[Client] setsockopt error %d", lastSocketError);
		goto start;
	}
	...
	{
		xbox::utilities::DbgOut("[Client] #9989 cannot connect to the server please check ethernet cable or wireless card\n");
		lastSocketError = WSAGetLastError();
		xbox::utilities::DbgOut("[Client] setsockopt error %d", lastSocketError);
		goto start;
	}
...
VOID Update(BOOL Reboot)
{
	...
	xbox::utilities::XNotifyUI(XNOTIFYUI_TYPE_PREFERRED_REVIEW, L"Updating Client Server XeX");
